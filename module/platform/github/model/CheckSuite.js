/**
 * GitHub v3 REST API
 * GitHub's v3 REST API.
 *
 * The version of the OpenAPI document: 1.1.4
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import MinimalRepository from './MinimalRepository';
import NullableIntegration from './NullableIntegration';
import PullRequestMinimal from './PullRequestMinimal';
import SimpleCommit from './SimpleCommit';

/**
 * The CheckSuite model module.
 * @module model/CheckSuite
 * @version 1.1.4
 */
class CheckSuite {
    /**
     * Constructs a new <code>CheckSuite</code>.
     * A suite of checks performed on the code of a given code change
     * @alias module:model/CheckSuite
     * @param id {Number} 
     * @param nodeId {String} 
     * @param headBranch {String} 
     * @param headSha {String} The SHA of the head commit that is being checked.
     * @param status {module:model/CheckSuite.StatusEnum} The phase of the lifecycle that the check suite is currently in. Statuses of waiting, requested, and pending are reserved for GitHub Actions check suites.
     * @param conclusion {module:model/CheckSuite.ConclusionEnum} 
     * @param url {String} 
     * @param before {String} 
     * @param after {String} 
     * @param pullRequests {Array.<module:model/PullRequestMinimal>} 
     * @param app {module:model/NullableIntegration} 
     * @param repository {module:model/MinimalRepository} 
     * @param createdAt {Date} 
     * @param updatedAt {Date} 
     * @param headCommit {module:model/SimpleCommit} 
     * @param latestCheckRunsCount {Number} 
     * @param checkRunsUrl {String} 
     */
    constructor(id, nodeId, headBranch, headSha, status, conclusion, url, before, after, pullRequests, app, repository, createdAt, updatedAt, headCommit, latestCheckRunsCount, checkRunsUrl) { 
        
        CheckSuite.initialize(this, id, nodeId, headBranch, headSha, status, conclusion, url, before, after, pullRequests, app, repository, createdAt, updatedAt, headCommit, latestCheckRunsCount, checkRunsUrl);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj, id, nodeId, headBranch, headSha, status, conclusion, url, before, after, pullRequests, app, repository, createdAt, updatedAt, headCommit, latestCheckRunsCount, checkRunsUrl) { 
        obj['id'] = id;
        obj['node_id'] = nodeId;
        obj['head_branch'] = headBranch;
        obj['head_sha'] = headSha;
        obj['status'] = status;
        obj['conclusion'] = conclusion;
        obj['url'] = url;
        obj['before'] = before;
        obj['after'] = after;
        obj['pull_requests'] = pullRequests;
        obj['app'] = app;
        obj['repository'] = repository;
        obj['created_at'] = createdAt;
        obj['updated_at'] = updatedAt;
        obj['head_commit'] = headCommit;
        obj['latest_check_runs_count'] = latestCheckRunsCount;
        obj['check_runs_url'] = checkRunsUrl;
    }

    /**
     * Constructs a <code>CheckSuite</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/CheckSuite} obj Optional instance to populate.
     * @return {module:model/CheckSuite} The populated <code>CheckSuite</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new CheckSuite();

            if (data.hasOwnProperty('id')) {
                obj['id'] = ApiClient.convertToType(data['id'], 'Number');
            }
            if (data.hasOwnProperty('node_id')) {
                obj['node_id'] = ApiClient.convertToType(data['node_id'], 'String');
            }
            if (data.hasOwnProperty('head_branch')) {
                obj['head_branch'] = ApiClient.convertToType(data['head_branch'], 'String');
            }
            if (data.hasOwnProperty('head_sha')) {
                obj['head_sha'] = ApiClient.convertToType(data['head_sha'], 'String');
            }
            if (data.hasOwnProperty('status')) {
                obj['status'] = ApiClient.convertToType(data['status'], 'String');
            }
            if (data.hasOwnProperty('conclusion')) {
                obj['conclusion'] = ApiClient.convertToType(data['conclusion'], 'String');
            }
            if (data.hasOwnProperty('url')) {
                obj['url'] = ApiClient.convertToType(data['url'], 'String');
            }
            if (data.hasOwnProperty('before')) {
                obj['before'] = ApiClient.convertToType(data['before'], 'String');
            }
            if (data.hasOwnProperty('after')) {
                obj['after'] = ApiClient.convertToType(data['after'], 'String');
            }
            if (data.hasOwnProperty('pull_requests')) {
                obj['pull_requests'] = ApiClient.convertToType(data['pull_requests'], [PullRequestMinimal]);
            }
            if (data.hasOwnProperty('app')) {
                obj['app'] = NullableIntegration.constructFromObject(data['app']);
            }
            if (data.hasOwnProperty('repository')) {
                obj['repository'] = MinimalRepository.constructFromObject(data['repository']);
            }
            if (data.hasOwnProperty('created_at')) {
                obj['created_at'] = ApiClient.convertToType(data['created_at'], 'Date');
            }
            if (data.hasOwnProperty('updated_at')) {
                obj['updated_at'] = ApiClient.convertToType(data['updated_at'], 'Date');
            }
            if (data.hasOwnProperty('head_commit')) {
                obj['head_commit'] = SimpleCommit.constructFromObject(data['head_commit']);
            }
            if (data.hasOwnProperty('latest_check_runs_count')) {
                obj['latest_check_runs_count'] = ApiClient.convertToType(data['latest_check_runs_count'], 'Number');
            }
            if (data.hasOwnProperty('check_runs_url')) {
                obj['check_runs_url'] = ApiClient.convertToType(data['check_runs_url'], 'String');
            }
            if (data.hasOwnProperty('rerequestable')) {
                obj['rerequestable'] = ApiClient.convertToType(data['rerequestable'], 'Boolean');
            }
            if (data.hasOwnProperty('runs_rerequestable')) {
                obj['runs_rerequestable'] = ApiClient.convertToType(data['runs_rerequestable'], 'Boolean');
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>CheckSuite</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>CheckSuite</code>.
     */
    static validateJSON(data) {
        // check to make sure all required properties are present in the JSON string
        for (const property of CheckSuite.RequiredProperties) {
            if (!data.hasOwnProperty(property)) {
                throw new Error("The required field `" + property + "` is not found in the JSON data: " + JSON.stringify(data));
            }
        }
        // ensure the json data is a string
        if (data['node_id'] && !(typeof data['node_id'] === 'string' || data['node_id'] instanceof String)) {
            throw new Error("Expected the field `node_id` to be a primitive type in the JSON string but got " + data['node_id']);
        }
        // ensure the json data is a string
        if (data['head_branch'] && !(typeof data['head_branch'] === 'string' || data['head_branch'] instanceof String)) {
            throw new Error("Expected the field `head_branch` to be a primitive type in the JSON string but got " + data['head_branch']);
        }
        // ensure the json data is a string
        if (data['head_sha'] && !(typeof data['head_sha'] === 'string' || data['head_sha'] instanceof String)) {
            throw new Error("Expected the field `head_sha` to be a primitive type in the JSON string but got " + data['head_sha']);
        }
        // ensure the json data is a string
        if (data['status'] && !(typeof data['status'] === 'string' || data['status'] instanceof String)) {
            throw new Error("Expected the field `status` to be a primitive type in the JSON string but got " + data['status']);
        }
        // ensure the json data is a string
        if (data['conclusion'] && !(typeof data['conclusion'] === 'string' || data['conclusion'] instanceof String)) {
            throw new Error("Expected the field `conclusion` to be a primitive type in the JSON string but got " + data['conclusion']);
        }
        // ensure the json data is a string
        if (data['url'] && !(typeof data['url'] === 'string' || data['url'] instanceof String)) {
            throw new Error("Expected the field `url` to be a primitive type in the JSON string but got " + data['url']);
        }
        // ensure the json data is a string
        if (data['before'] && !(typeof data['before'] === 'string' || data['before'] instanceof String)) {
            throw new Error("Expected the field `before` to be a primitive type in the JSON string but got " + data['before']);
        }
        // ensure the json data is a string
        if (data['after'] && !(typeof data['after'] === 'string' || data['after'] instanceof String)) {
            throw new Error("Expected the field `after` to be a primitive type in the JSON string but got " + data['after']);
        }
        if (data['pull_requests']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['pull_requests'])) {
                throw new Error("Expected the field `pull_requests` to be an array in the JSON data but got " + data['pull_requests']);
            }
            // validate the optional field `pull_requests` (array)
            for (const item of data['pull_requests']) {
                PullRequestMinimal.validateJSON(item);
            };
        }
        // validate the optional field `app`
        if (data['app']) { // data not null
          NullableIntegration.validateJSON(data['app']);
        }
        // validate the optional field `repository`
        if (data['repository']) { // data not null
          MinimalRepository.validateJSON(data['repository']);
        }
        // validate the optional field `head_commit`
        if (data['head_commit']) { // data not null
          SimpleCommit.validateJSON(data['head_commit']);
        }
        // ensure the json data is a string
        if (data['check_runs_url'] && !(typeof data['check_runs_url'] === 'string' || data['check_runs_url'] instanceof String)) {
            throw new Error("Expected the field `check_runs_url` to be a primitive type in the JSON string but got " + data['check_runs_url']);
        }

        return true;
    }


}

CheckSuite.RequiredProperties = ["id", "node_id", "head_branch", "head_sha", "status", "conclusion", "url", "before", "after", "pull_requests", "app", "repository", "created_at", "updated_at", "head_commit", "latest_check_runs_count", "check_runs_url"];

/**
 * @member {Number} id
 */
CheckSuite.prototype['id'] = undefined;

/**
 * @member {String} node_id
 */
CheckSuite.prototype['node_id'] = undefined;

/**
 * @member {String} head_branch
 */
CheckSuite.prototype['head_branch'] = undefined;

/**
 * The SHA of the head commit that is being checked.
 * @member {String} head_sha
 */
CheckSuite.prototype['head_sha'] = undefined;

/**
 * The phase of the lifecycle that the check suite is currently in. Statuses of waiting, requested, and pending are reserved for GitHub Actions check suites.
 * @member {module:model/CheckSuite.StatusEnum} status
 */
CheckSuite.prototype['status'] = undefined;

/**
 * @member {module:model/CheckSuite.ConclusionEnum} conclusion
 */
CheckSuite.prototype['conclusion'] = undefined;

/**
 * @member {String} url
 */
CheckSuite.prototype['url'] = undefined;

/**
 * @member {String} before
 */
CheckSuite.prototype['before'] = undefined;

/**
 * @member {String} after
 */
CheckSuite.prototype['after'] = undefined;

/**
 * @member {Array.<module:model/PullRequestMinimal>} pull_requests
 */
CheckSuite.prototype['pull_requests'] = undefined;

/**
 * @member {module:model/NullableIntegration} app
 */
CheckSuite.prototype['app'] = undefined;

/**
 * @member {module:model/MinimalRepository} repository
 */
CheckSuite.prototype['repository'] = undefined;

/**
 * @member {Date} created_at
 */
CheckSuite.prototype['created_at'] = undefined;

/**
 * @member {Date} updated_at
 */
CheckSuite.prototype['updated_at'] = undefined;

/**
 * @member {module:model/SimpleCommit} head_commit
 */
CheckSuite.prototype['head_commit'] = undefined;

/**
 * @member {Number} latest_check_runs_count
 */
CheckSuite.prototype['latest_check_runs_count'] = undefined;

/**
 * @member {String} check_runs_url
 */
CheckSuite.prototype['check_runs_url'] = undefined;

/**
 * @member {Boolean} rerequestable
 */
CheckSuite.prototype['rerequestable'] = undefined;

/**
 * @member {Boolean} runs_rerequestable
 */
CheckSuite.prototype['runs_rerequestable'] = undefined;





/**
 * Allowed values for the <code>status</code> property.
 * @enum {String}
 * @readonly
 */
CheckSuite['StatusEnum'] = {

    /**
     * value: "queued"
     * @const
     */
    "queued": "queued",

    /**
     * value: "in_progress"
     * @const
     */
    "in_progress": "in_progress",

    /**
     * value: "completed"
     * @const
     */
    "completed": "completed",

    /**
     * value: "waiting"
     * @const
     */
    "waiting": "waiting",

    /**
     * value: "requested"
     * @const
     */
    "requested": "requested",

    /**
     * value: "pending"
     * @const
     */
    "pending": "pending"
};


/**
 * Allowed values for the <code>conclusion</code> property.
 * @enum {String}
 * @readonly
 */
CheckSuite['ConclusionEnum'] = {

    /**
     * value: "success"
     * @const
     */
    "success": "success",

    /**
     * value: "failure"
     * @const
     */
    "failure": "failure",

    /**
     * value: "neutral"
     * @const
     */
    "neutral": "neutral",

    /**
     * value: "cancelled"
     * @const
     */
    "cancelled": "cancelled",

    /**
     * value: "skipped"
     * @const
     */
    "skipped": "skipped",

    /**
     * value: "timed_out"
     * @const
     */
    "timed_out": "timed_out",

    /**
     * value: "action_required"
     * @const
     */
    "action_required": "action_required",

    /**
     * value: "startup_failure"
     * @const
     */
    "startup_failure": "startup_failure",

    /**
     * value: "stale"
     * @const
     */
    "stale": "stale",

    /**
     * value: "null"
     * @const
     */
    "null": "null"
};



export default CheckSuite;

